...
class MarkItDown:
        ...
        try:
            if file_path.suffix.lower() == ".docx":
                ...
                raise ConversionError(
                    f"No converter implemented for: {file_path.suffix}"
                )
    ...
    def _convert_html(self, file_path: Path) -> str:
        ...
        try:
            ...
            markdown = re.sub(
                r"\s+\|\s+", " | ", markdown
            )  # Normalize table cell spacing
            markdown = re.sub(r"\n\s*\n+", "\n\n", markdown)  # Remove extra blank lines
            markdown = re.sub(
                r"(\n\+[-]+\+)+\n", "\n", markdown
            )  # Remove redundant table borders
            markdown = re.sub(
                r"(\n\|[-]+\|)+\n", "\n", markdown
            )  # Remove redundant table separators

            # Clean up JIRA-specific formatting
            markdown = re.sub(
                r"\[(?:Reopened|In Progress|Awaiting Approval|Need Business Feedback)\]",
                r"\g<0>",
                markdown,
            )  # Keep status without formatting
            markdown = re.sub(
                r"\[\]\{[^}]*\}", "", markdown
            )  # Remove empty links with attributes
            markdown = re.sub(
                r":::\s*\{[^}]*\}[^:]*:::", "", markdown
            )  # Remove JIRA tooltips
            markdown = re.sub(
                r":::\s*end-of-[^:]*:::", "", markdown
            )  # Remove JIRA end markers
